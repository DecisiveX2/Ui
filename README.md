local continue = continue or (syn and continue) or (function()
    end)
...
do
    ...
    function Funtions:AddColorPicker(Idx, Info)
...
function Library:CreateWindow(...)
    ...
    function Window:AddTab(Name)
        ...
        local RightSide =
            ...
            "ScrollingFrame",
            {
                ...
                Size = UDim2.new(0.5, -12 + 2, 1, -15), -- 507 + 2
                CanvasSize = UDim2.new(0, 0, 0, 0),
                BottomImage = "",
                TopImage = "",
                ScrollBarThickness = 0,
        ...
        for _, Side in next, {LeftSide, RightSide} do
            Side:WaitForChild("UIListLayout"):GetPropertyChangedSignal("AbsoluteContentSize"):Connect(
                function()
    ...
    function Library:Toggle()
        ...
        if Toggled then
            ...
            if Library.ScreenScaling then -- This is mouse cursor you can disable it
                task.spawn(
                    function()
                ...
                if type(Library.ToggleKeybind) == "table" and Library.ToggleKeybind.Type == "KeyPicker" then
                    if
                        Input.UserInputType == Enum.UserInputType.Keyboard and
                            Input.KeyCode.Name == Library.ToggleKeybind.Value
                     then
                        task.spawn(Library.Toggle)
                    end
                elseif
                    Input.KeyCode == Enum.KeyCode.RightControl or
                        (Input.KeyCode == Enum.KeyCode.RightShift and (not Processed))
                 then
                    task.spawn(Library.Toggle)
                end
            end
...
Library.ItsConfig = function()
    return getgenv().GlobalConfigs
end

Library.SaveConfig = function(...)
    local p = {...}
    if type(p[1]) == "table" and isfile(tostring(p[2])) then
        writefile(tostring(p[2]), game:GetService("HttpService"):JSONEncode(p[1]))
    end
end

Library.LoadConfig = function(...)
    local p = {...}
    if isfile(tostring(p[2])) then
        local file = game:GetService("HttpService"):JSONDecode(readfile(tostring(p[2])))
        for i, v in next, p[1] do
            if not file[i] then
                file[i] = v
                writefile(tostring(p[2]), game:GetService("HttpService"):JSONEncode(file))
            end
        end
        p[1] = file
        getgenv().GlobalConfigs = file
        if p[3] then
            return p[1]
        end
    else
        makefolder(tostring(p[2]):split("/")[1])
        writefile(tostring(p[2]), game:GetService("HttpService"):JSONEncode(p[1]))
    end
end
